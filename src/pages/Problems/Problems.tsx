  ...
  return (
    <div className={styles.problems}>
      <div className={styles.header}>
        ...
        {isLoggedIn && (
          <Link to="/problems/new" className={styles.addButton}>
            + 추가
          </Link>
        )}
      </div>
        ...
        {problems.map((problem) => (
          <div key={problem.id} className={styles.card}>
            <div className={styles.cardTop}>
              ...
              <span
                className={`${styles.difficulty} ${styles[problem.difficulty]}`}
              >
                {problem.difficulty}
              </span>
              <span
                className={`${styles.status} ${styles[problem.status === "풀이 완료" ? "done" : problem.status === "풀이 중" ? "progress" : "retry"]}`}
              >
                {problem.status}
              </span>
            </div>
